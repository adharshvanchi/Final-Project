...
Using the data we just gathered and discussed, we must first show the trends of the data as a whole, using World data to determine whether the countries we explore further are above or below the world average.
First we consolidate the disparate dataframes into one `wld_data` Dataframe. This allows us to manipulate and graph the data.
...
First, lets compare the Cold War Rivals, The United States vs Russia.


First we create US and RU dataframes to combine and create a World-US-Russia dataframe to make comparisons
```{r US-data}
...
First we compare the Populations of Russia and US
```{r pop_1}
ggplot(WUR_data %>% filter(Entity != "World"))+
  geom_col(aes(Year,pop,color = Entity), position = "dodge")
```
Notice that since the end of the Cold War in 1991, The population has generally trended lower for Russia and drastically higher for the US. 
...
```
Notice that while the US has had 70% or above since 2005, while Russia only broke 70% a full decade later. Their victory of the Cold War allowed the US to spend more resources implementing infrastructural upgrades while Russia had to take care of more essential needs. But Russia has more mobile users, which seems to be due to the inherent lower cost of cellular network installation and operation, for both the providers and consumers.
...
```
As arguably the largest economic force ever in history, it is no surprise to see the United States far outpacing its former rival. And their private and government actions to further internet access has only increased their output. Russia is no slouch, however, pulling its GDP per Capita above the World Average after the 2008 financial crisis.
```{r}
ggplot(WUR_data %>% filter(Entity != "World"))+
  geom_col(aes(Year,ALO, color = Entity), position = "dodge")
  
```
While Russia technically scores higher, the missing data is the number of students this study takes into account and how small the difference is despite the massive difference in population. Despite having similar ALO scores, the USA far outpaces Russia in GDP, and overall internet coverage.

Moving onto South Korea and Myanmar, both these countries are located in Asia and have similar populations at the end of the dataset(2016). By comparing them, we can compare more precisely and draw stronger conclusions.
...
```
Graphing the GDP per Capita, it is easy to see the wealth disparity between these two nations. Similar to their internet infrastructure, Myanmar cannot compare against South Korea, despite the similar populations. 
```{r}
ggplot(NULL, aes(Year,ALO))+
  geom_point(data = US_data, color = "Blue")+
  geom_point(data = SK_data)
  
```
Myanmar does not have any ALO information, this may be due to the precarious governmental organization there, but South Korea has consistently scored higher than even the United States, so it would suffice it to say South Korea is far more educated than Myanmar based on the previous analysis. This shows that not only does internet access beget wealth, but wealth begets internet access.

Finally, Two countries with similar populations with access to Internet
